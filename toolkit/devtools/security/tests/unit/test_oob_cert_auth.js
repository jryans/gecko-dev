...
add_task(function*() {
  ...
  listener.authentication = DebuggerServer.Authentication.OOB_CERT;
  listener.receiveOOB = () => oobData.promise; // Skip prompt for tests
  yield listener.open();
  ...
  let transport = yield DebuggerClient.socketConnect({
    ...
    port: listener.port,
    encryption: true,
    authentication: DebuggerClient.Authentication.OOB_CERT,
    cert: {
...
add_task(function*() {
  ...
  let transport = yield DebuggerClient.socketConnect({
    ...
    port: listener.port,
    encryption: true
    // authentication: PROMPT is the default
  });
