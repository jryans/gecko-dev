...
/**
 * Notify the user about the state of the connection in progress.
 *
 ...
 */
Client.defaultNotification = ({ authentication, authResult, oob }) => {
  // Only show something with OOB_CERT in the PENDING state
  if (authentication != DebuggerSocket.Authentication.OOB_CERT ||
      authResult != DebuggerSocket.AuthenticationResult.PENDING) {
    return;
  }
  const key = "clientPendingOOB";
  let title = bundle.GetStringFromName(key + "Title");
  let header = bundle.GetStringFromName(key + "Header");
  let hashMsg = bundle.formatStringFromName(key + "Hash",
                                            [oob.sha256], 1);
  let token = oob.sha256.replace(/:/g, "").toLowerCase() + oob.k;
  let tokenMsg = bundle.formatStringFromName(key + "Token",
                                             [token], 1);
  let msg =`${header}\n\n${hashMsg}\n${tokenMsg}`;
...
Server.defaultReceiveOOB = () => {
  ...
  return promise.resolve({ sha256, k });
};
