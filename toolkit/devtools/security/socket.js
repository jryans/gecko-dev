 ...
 * @param prompt object (optional)
 *        Override default prompts to customize UX.
 *        TODO: Too many objects, etc.
 * @return promise
...
DebuggerSocket.connect = Task.async(function*(settings) {
  ...
  if (authentication == Authentication.PROMPT) {
    // PROMPT authentication is purely server-side
    return transport;
  }
  ...
  let oob;
  let showPrompt = settings.prompt.sendOOB ||
                   prompt.Client.defaultSendOOB;
  let activePrompt;
  transport.hooks = {
    onPacket: Task.async(function*(packet) {
      ...
      switch (authResult) {
        case AuthenticationResult.PENDING:
          ...
          oob = yield createOOB();
          let details = Object.assign({}, settings);
...
ServerSocketConnection.prototype = {
  ...
  _onHandshakeTimeout() {
    ...
    this._handshakeDeferred.reject("NS_ERROR_NET_TIMEOUT");
  },
  ...
  onHandshakeDone(socket, clientStatus) {
    ...
    if (clientStatus.tlsVersionUsed != Ci.nsITLSClientStatus.TLS_VERSION_1_2) {
      this._handshakeDeferred.reject("NS_ERROR_CONNECTION_REFUSED");
      return;
    }
  ...
  deny(errorName) {
    dumpn("Debugging connection denied on " + this.address +
          " (" + errorName + ")");
    let result = Cr[errorName] ? Cr[errorName] : Cr.NS_ERROR_FAILURE;
