...
DebuggerClient.prototype = {
   ...
   *           using the event data object containing:
   *           * actor:  Actor that received the packet
   *           * type:   Method of the actor that was called on receipt
   ...
   *                     below), you must signal completion by resolving /
   *                     rejecting this deferred.  If you do use |copyTo|, this
   *                     is taken care of for you when copying completes.
  ...
  startBulkRequest: function(request) {
    ...
    if (!request.actor) {
      throw Error("'" + type + "' bulk packet has no destination.");
    }
    if (!request.length) {
      throw Error("'" + type + "' bulk packet has no length.");
    }
...
function Request(request) {
  ...
  this.off = events.off.bind(this, this);
