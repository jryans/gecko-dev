...
let App = createClass({
  ...
  onChangeDevice(id, deviceType, device) {
    window.postMessage({
      type: "change-device",
      device,
    }, "*");
    this.props.dispatch(changeDevice(id, deviceType, device.name));
  ...
  render() {
    ...
    let modalViewportTemplate = {};
    if (devices.modalOpenedFromViewport !== null) {
      modalViewportTemplate = viewports[devices.modalOpenedFromViewport];
    }
