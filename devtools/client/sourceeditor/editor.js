...
Editor.prototype = {
  ...
  appendTo: function(el, env) {
    ...
    let onLoad = () => {
      ...
      // to "tear off" and swim around on top of the content.  Bug 1160601
      // tracks finding a solution that allows APZ to work with CodeMirror.
      cm.getScrollerElement().addEventListener("wheel", ev => {
        // By handling the wheel events ourselves, we force the platform to
        // scroll synchronously, like it did before APZ.  However, we lose
        // smooth scrolling for users with mouse wheels.  This seems acceptible
        // vs. doing nothing and letting the gutter slide around.
        ev.preventDefault();
        ...
        if (ev.deltaMode == ev.DOM_DELTA_LINE) {
          ...
          deltaX *= cm.getScrollerElement().clientWidth;
          deltaY *= cm.getScrollerElement().clientHeight;
        }
