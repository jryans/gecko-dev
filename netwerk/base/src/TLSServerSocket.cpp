...
typedef void (TLSServerSocket:: *TLSServerSocketFunc)(void);

static nsresult
PostEvent(TLSServerSocket *s, TLSServerSocketFunc func)
{
...
void
TLSServerSocket::OnMsgClose()
{
  SOCKET_LOG(("TLSServerSocket::OnMsgClose [this=%p]\n", this));
...
void
TLSServerSocket::OnMsgAttach()
{
  SOCKET_LOG(("TLSServerSocket::OnMsgAttach [this=%p]\n", this));
...
TLSServerSocket::TryAttach()
{
  ...
  if (!gSocketTransportService->CanAttachSocket())
  {
    nsCOMPtr<nsIRunnable> event =
      NS_NewRunnableMethod(this, &TLSServerSocket::OnMsgAttach);
    if (!event)
      return NS_ERROR_OUT_OF_MEMORY;
...
//-----------------------------------------------------------------------------
// TLSServerSocket::nsASocketHandler
//-----------------------------------------------------------------------------

void
TLSServerSocket::OnSocketReady(PRFileDesc *fd, int16_t outFlags)
{
...
void
TLSServerSocket::IsLocal(bool *aIsLocal)
{
#if defined(XP_UNIX)
...
void
TLSServerSocket::KeepWhenOffline(bool *aKeepWhenOffline)
{
...
NS_IMETHODIMP
TLSServerSocket::Init(int32_t aPort, bool aLoopbackOnly, int32_t aBackLog)
{
  return InitSpecialConnection(aPort, aLoopbackOnly ? LoopbackOnly : 0, aBackLog);
}

NS_IMETHODIMP
TLSServerSocket::InitWithFilename(nsIFile *aPath, uint32_t aPermissions, int32_t aBacklog)
{
#if defined(XP_UNIX)
  nsresult rv;
...
NS_IMETHODIMP
TLSServerSocket::InitSpecialConnection(int32_t aPort, nsServerSocketFlag aFlags,
                                      int32_t aBackLog)
{
  PRNetAddrValue val;
  PRNetAddr addr;
...
NS_IMETHODIMP
TLSServerSocket::InitWithAddress(const PRNetAddr *aAddr, int32_t aBackLog)
{
...
TLSServerSocket::Close()
{
  ...
  return PostEvent(this, &TLSServerSocket::OnMsgClose);
}
...
NS_IMETHODIMP
TLSServerSocket::AsyncListen(nsIServerSocketListener *aListener)
{
...
NS_IMETHODIMP
TLSServerSocket::GetPort(int32_t *aResult)
{
...
NS_IMETHODIMP
TLSServerSocket::GetAddress(PRNetAddr *aResult)
{
  // no need to enter the lock here
