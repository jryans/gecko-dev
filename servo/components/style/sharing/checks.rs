...
pub fn parents_allow_sharing<E>(target: &mut StyleSharingTarget<E>,
                                candidate: &mut StyleSharingCandidate<E>) -> bool
    where E: TElement,
{
    // If the identity of the parent style isn't equal, we can't share.
    ...
    let candidate_parent = match candidate.element.inheritance_parent() {
        Some(p) => p,
        None => return false,
    };
